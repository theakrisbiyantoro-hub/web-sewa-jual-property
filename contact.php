...
if ($_SERVER["REQUEST_METHOD"] === "POST" && ($_POST["action"] ?? "") === "send_contact") {
    ...
    if ($full_name === "" || $email === "" || $message === "") {
        $contactError = "Semua field wajib diisi.";
    } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $contactError = "Email tidak valid.";
    } else {
        if ($uid === null) {
            $stmt = $koneksi->prepare("INSERT INTO contact (user_id, full_name, email, message) VALUES (NULL, ?, ?, ?)");
            $stmt->bind_param("sss", $full_name, $email, $message);
        } else {
            $stmt = $koneksi->prepare("INSERT INTO contact (user_id, full_name, email, message) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("isss", $uid, $full_name, $email, $message);
        }

        if ($stmt && $stmt->execute()) {
            $contactSuccess = "Pesan terkirim! Admin akan segera menghubungi Anda. Terima kasih!";
        } else {
            $contactError = "Gagal mengirim pesan. Coba lagi.";
        }
    }
}
?>
...
<title>Contact Us | PropertyKu</title>
...
<style>
    * {
    ...
    .contact-hero {
        height: 55vh;
        background: linear-gradient(rgba(29, 36, 51, 0.55),
                rgba(29, 36, 51, 0.55)),
    ...
    .profile-logout{display:block;margin-top:8px;text-align:center;color:#ffb4b4;text-decoration:none;}
    .profile-edit-btn{
        width:100%;
        padding:10px;
        border-radius:10px;
    ...
    <div class="contact-wrapper">
        ...
        <div class="contact-info">
            ...
            <div class="contact-item">
                <i class="ri-phone-line"></i> +62 812-3456-7890
            </div>
            <div class="contact-item">
                <i class="ri-mail-line"></i> support@propertyku.com
            </div>
            <div class="contact-item">
                <i class="ri-map-pin-line"></i> Surabaya, East Java
            </div>
        </div>

        <form class="contact-form" method="POST" action="">
            <?php if ($contactSuccess): ?>
                <div style="margin-bottom:12px;padding:12px;border-radius:12px;background:#e7f8ee;color:#116b3a;">
                    <?= htmlspecialchars($contactSuccess) ?>
                </div>
                <?php elseif ($contactError): ?>
                <div style="margin-bottom:12px;padding:12px;border-radius:12px;background:#ffe9e9;color:#8a1f1f;">
            ...
            <div class="form-group">
                ...
                <input type="text" name="full_name" required value="<?= htmlspecialchars($userName ?? "") ?>">
            </div>

            <div class="form-group">
                <label>Email</label>
                <input type="email" name="email" required value="<?= htmlspecialchars($userEmail ?? "") ?>">
            </div>
...
<section class="map-section">
    <div class="map-container">
        <iframe src="https://www.google.com/maps?q=Surabaya&output=embed">
        </iframe>
    </div>
</section>
