...
function file_url($base, $file)
{
    return $base . ltrim((string)$file, "/");
}
...
// ambil property (FULL)
$p = $koneksi->prepare("SELECT * FROM property WHERE id=?");
$p->bind_param("i", $property_id);
$p->execute();
$property = $p->get_result()->fetch_assoc();
$p->close();
if (!$property) die("Property tidak ditemukan");

// ===== ambil harga per tipe unit (property_apart_harga) =====
$hargaTipeRows = [];
$hargaMap = [];

$stmtHarga = $koneksi->prepare("SELECT nama_tipe, harga FROM property_apart_harga WHERE id_property=? ORDER BY id ASC");
$stmtHarga->bind_param("i", $property_id);
$stmtHarga->execute();
$hargaTipeRows = $stmtHarga->get_result()->fetch_all(MYSQLI_ASSOC);
$stmtHarga->close();

// helper normalisasi nama tipe biar match (misal "1 BR", "1BR", "1-bedroom")
function norm_tipe($s)
{
...
foreach ($hargaTipeRows as $row) {
    $k = norm_tipe($row['nama_tipe'] ?? '');
    $v = (int)($row['harga'] ?? 0);
    if ($k !== '' && $v > 0) $hargaMap[$k] = $v;
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["do_booking"])) {
    ...
    if ($full_name === "" || $email === "" || $phone === "" || $unit_type === "" || $checkin_date === "") {
        ...
        // ambil harga sesuai unit
        $price = 0;
        $key = norm_tipe($unit_type);
        if (isset($hargaMap[$key])) {
            $price = $hargaMap[$key];
        }

        $stmt = $koneksi->prepare("
  INSERT INTO booking 
  (user_id, property_id, full_name, email, phone, unit_type, checkin_date, total_price, status, created_at)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'wait_payment', NOW())
");
        ...
        if ($stmt->execute()) {
            ...
            $message = "Hello Admin, I want to book a unit.\n\n"
...
<body>
    ...
    <div class="hero-slider">
        ...
        <div class="slide active">
            <img src="img/pexels-heyho-22 (3).jpg">
        </div>
    ...
    <section class="apartment-detail">
        <div class="apartment-wrapper">
            ...
            <div class="apartment-image">
                <img src="<?= esc(file_url($BASE_UPLOAD, $property['cover'])) ?>" alt="Apartement Image">
            </div>
            ...
            <div class="apartment-info">
                ...
                <div class="apartment-spec">
                    ...
                    <div>
                        <span>Status</span>
                        <strong class="status available"><?= esc($property['status']) ?></strong>
                    </div>
                </div>
        ...
        <div class="floorplan-wrapper">
            ...
            <div class="floorplan-info">
                <h2 class="floorplan-title">
                    Floor Plan <span class="title-apartment">Apartment</span>
                </h2>
            ...
            <div class="floorplan-image">
                <img src="<?= esc(file_url($BASE_UPLOAD, $property['denah'])) ?>" alt="Apartment Floor Plan">
            </div>
    ...
    <section class="facility-section">
        ...
        <div class="facility-slider" id="facilitySlider">
            <?php if (empty($media["FASILITAS"])): ?>
                <p style="padding:10px">Belum ada fasilitas.</p>
            <?php else: ?>
                <?php foreach ($media["FASILITAS"] as $f): ?>
                    <div class="facility-card">
                        <img src="<?= esc(file_url($BASE_UPLOAD, $f['file'])) ?>" alt="<?= esc($f['nama'] ?? 'Facility') ?>">
                        <p><?= esc($f['nama'] ?? 'Facility') ?></p>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
        </div>
    ...
    <!-- ================= UNIT DAN LAYOUT ================= -->
    <section class="unit-layout-section">

        <div class="unit-layout-wrapper">

            <!-- LEFT : FLOORPLAN IMAGE -->
            <div class="unit-image">
                <h2 class="unit-image-title" id="unitImageTitle">
                    Unit Layout
                </h2>
                <img id="unitImage"
                    src="<?= esc(!empty($media['UNIT_LAYOUT']) ? file_url($BASE_UPLOAD, $media['UNIT_LAYOUT'][0]['file']) : file_url($BASE_UPLOAD, $property['cover'])) ?>"
                    alt="Unit Layout">
                <a href="<?= esc(!empty($media['PANORAMA']) ? file_url($BASE_UPLOAD, $media['PANORAMA'][0]['file']) : '#') ?>"
                    target="_blank" class="unit-360-btn" id="panoramaBtn"><i class="ri-360-line"></i>View 360° Panorama</a>
            </div>

            <!-- RIGHT : UNIT INFO -->
            <div class="unit-info">

                <!-- UNIT TYPE TABS (from DB) -->
                <?php if (empty($hargaTipeRows)): ?>
                    <p style="opacity:.7; margin:10px 0;">
                        Data tipe unit belum tersedia.
                    </p>
                <?php else: ?>
                    <?php foreach ($hargaTipeRows as $i => $h):
                        $namaTipe = $h['nama_tipe'];
                        $key = norm_tipe($namaTipe);
                        $hargaUnit = (int)$h['harga'];

                        // cari layout yang cocok dengan tipe ini
                        $u = null;
                        foreach ($media["UNIT_LAYOUT"] as $lu) {
                            if (norm_tipe($lu['nama'] ?? '') === $key) {
                                $u = $lu;
                                break;
                            }
                        }

                        // parse spesifikasi (kalau layout ada)
                        $spec = $u ? parse_unit_desc($u["deskripsi"] ?? "") : [
                            "area" => "-",
                            "bedrooms" => "-",
                            "bathrooms" => "-",
                            "ceiling" => "-",
                        ...
                        $img = $u
                            ? file_url($BASE_UPLOAD, $u['file'])
                            : file_url($BASE_UPLOAD, $property['cover']);
                    ?>
                        <button class="unit-tab <?= $i === 0 ? 'active' : '' ?>"
                            data-title="<?= esc($namaTipe) ?>"
                            data-image="<?= esc($img) ?>"

                            data-area="<?= esc($spec['area']) ?>"
                            data-bedroom="<?= esc($spec['bedrooms']) ?>"
                            data-bathroom="<?= esc($spec['bathrooms']) ?>"
                            data-ceiling="<?= esc($spec['ceiling']) ?>"

                            data-price="<?= esc(rupiah($hargaUnit)) ?>"
                            data-panorama="<?= esc(!empty($media['PANORAMA']) ? file_url($BASE_UPLOAD, $media['PANORAMA'][0]['file']) : '#') ?>">
                            <?= esc($namaTipe) ?>
                        </button>
                    <?php endforeach; ?>
                <?php endif; ?>

                <h2 class="unit-title" id="unitTitle">
                    <?= esc($hargaTipeRows[0]['nama_tipe'] ?? 'Unit Layout') ?>
                </h2>

                <div class="unit-layout-list" id="unitLayoutList">
                    <div class="layout-row"><span>Total Area</span><strong>-</strong></div>
                    <div class="layout-row"><span>Bedrooms</span><strong>-</strong></div>
                    <div class="layout-row"><span>Bathrooms</span><strong>-</strong></div>
                    <div class="layout-row"><span>Ceiling Height</span><strong>-</strong></div>
                    <div class="layout-row highlight"><span>Total Price</span><strong><?= esc(rupiah($hargaTipeRows[0]['harga'] ?? 0)) ?></strong>
                    </div>
    ...
    <section class="unit-spec-section">
        ...
        <div class="unit-spec-wrapper">
            ...
            <div class="unit-spec-header">
                ...
                <h2 class="unit-spec-title">Unit Specifications</h2>
            </div>
            ...
            // tipe unit = unik dari SPEC.nama
            $specTypes = [];
            foreach ($media["SPEC"] as $s) {
                $t = trim((string)($s["nama"] ?? ""));
                if ($t !== "") $specTypes[$t] = true;
            }
            $specTypes = array_keys($specTypes);
            $activeType = $specTypes[0] ?? "";
            ?>

            <div class="spec-unit-switch" id="specUnitSwitch">
                <?php foreach ($specTypes as $i => $t): ?>
                    <button type="button"
                        class="spec-unit-btn <?= $i === 0 ? 'active' : '' ?>"
                        data-type="<?= esc(slug(norm_tipe($t))) ?>">
                        <?= esc($t) ?>
                    </button>
                <?php endforeach; ?>
            </div>
            ...
            <div class="unit-spec-image">
                <img src="<?= esc(!empty($media['SPEC']) ? file_url($BASE_UPLOAD, $media['SPEC'][0]['file']) : file_url($BASE_UPLOAD, $property['cover'])) ?>"
                    alt="Unit Interior">
                <button class="unit-nav next" id="specNextBtn">
                    <span id="nextLabel">-</span>
                    <i class="ri-arrow-right-s-line"></i>
                </button>

                <button class="unit-nav prev" id="specPrevBtn">
            ...
            <!-- BUTTONS -->
            <?php
            // kategori = SPEC.deskripsi, dikelompokkan per tipe (SPEC.nama)
            $catsByType = []; // [typeKey => [catKey => ['label'=>..., 'img'=>...]]]

            foreach ($media["SPEC"] as $s) {

                $rawType = trim((string)($s['nama'] ?? ''));        // TIPE
                $rawCat  = trim((string)($s['deskripsi'] ?? ''));   // KATEGORI

                if ($rawType === '') continue;
                if ($rawCat === '') $rawCat = 'Default';

                $typeKey = slug(norm_tipe($rawType));
                $catKey  = slug($rawCat);

                // simpan 1 gambar pertama untuk kategori itu (representatif)
                if (!isset($catsByType[$typeKey][$catKey])) {
                    $catsByType[$typeKey][$catKey] = [
                        'label' => $rawCat,
                        'img'   => file_url($BASE_UPLOAD, $s['file']),
                    ];
                }
            }
            ?>

            <div class="spec-buttons" id="specCategoryButtons">
                <?php if (empty($catsByType)): ?>
                    <p style="padding:10px">Belum ada foto spesifikasi unit.</p>
                <?php else: ?>
                    <?php foreach ($catsByType as $typeKey => $cats): ?>
                        <?php foreach ($cats as $catKey => $meta): ?>
                            <button class="spec-btn"
                                data-type="<?= esc($typeKey) ?>"
                                data-cat="<?= esc($catKey) ?>"
                                data-title="<?= esc($meta['label']) ?>"
                                data-img="<?= esc($meta['img']) ?>">
                                <?= esc($meta['label']) ?>
                            </button>
                        <?php endforeach; ?>
                    <?php endforeach; ?>
    ...
    <section class="public-domain-section">
        ...
        <div class="public-header">
            <div>
                <h2>Public<br>Domain</h2>
                <p>Facilities available inside the apartment area</p>
            </div>

            <div class="public-nav">
                <button id="publicPrev"><i class="ri-arrow-left-line"></i></button>
                <button id="publicNext"><i class="ri-arrow-right-line"></i></button>
            </div>
        </div>

        <div class="public-slider" id="publicSlider">
            <?php if (empty($media["PUBLIC_DOMAIN"])): ?>
                <p style="padding:10px">Belum ada public domain.</p>
            <?php else: ?>
                <?php foreach ($media["PUBLIC_DOMAIN"] as $pd): ?>
                    <div class="public-item">
                        <img src="<?= esc(file_url($BASE_UPLOAD, $pd['file'])) ?>">
                        <div class="public-info">
                            <h4><?= esc($pd['nama'] ?? 'Public Place') ?></h4>
                            <span><?= esc($pd['deskripsi'] ?? '') ?></span>
                        </div>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
    ...
    <section class="location-section">
        <div class="location-wrapper">
            ...
            <div class="location-header">
                ...
                <h2 class="location-title">Apartment Location</h2>
        ...
        <div class="booking-wrapper">
            ...
            <form class="booking-form" id="bookingForm" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select id="unitTypeSelect" name="unit_type" required></select>
                </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            const facilityScroll = 460;

            if (facilitySlider && facilityNext && facilityPrev) {
                facilityNext.addEventListener("click", () => facilitySlider.scrollLeft += facilityScroll);
                facilityPrev.addEventListener("click", () => facilitySlider.scrollLeft -= facilityScroll);
            }

            /* ================= UNIT TABS (layout) ================= */
            const tabs = Array.from(document.querySelectorAll(".unit-tab"));
            const unitImage = document.getElementById("unitImage");
            const unitLayoutList = document.getElementById("unitLayoutList");
            const unitTitle = document.getElementById("unitTitle");
            const panoramaBtn = document.getElementById("panoramaBtn");

            function renderUnitFromTab(tab) {
                if (!tab) return;

                tabs.forEach(t => t.classList.remove("active"));
                tab.classList.add("active");

                if (unitImage && tab.dataset.image) unitImage.src = tab.dataset.image;
                if (unitTitle && tab.dataset.title) unitTitle.textContent = tab.dataset.title;
                if (panoramaBtn && tab.dataset.panorama) panoramaBtn.href = tab.dataset.panorama;

                if (unitLayoutList) {
                    unitLayoutList.innerHTML = `
                <div class="layout-row"><span>Total Area</span><strong>${tab.dataset.area || "-"}</strong></div>
                <div class="layout-row"><span>Bedrooms</span><strong>${tab.dataset.bedroom || "-"}</strong></div>
                <div class="layout-row"><span>Bathrooms</span><strong>${tab.dataset.bathroom || "-"}</strong></div>
                <div class="layout-row"><span>Ceiling Height</span><strong>${tab.dataset.ceiling || "-"}</strong></div>
                <div class="layout-row highlight"><span>Total Price</span><strong>${tab.dataset.price || "-"}</strong></div>
            `;
                }
            }

            tabs.forEach(tab => tab.addEventListener("click", () => renderUnitFromTab(tab)));
            if (tabs.length) renderUnitFromTab(tabs[0]);
            ...
            const publicScroll = 350;

            if (publicSlider && publicNext && publicPrev) {
                publicNext.addEventListener("click", () => publicSlider.scrollLeft += publicScroll);
                publicPrev.addEventListener("click", () => publicSlider.scrollLeft -= publicScroll);
            }

            /* ================= AUTO UNIT TYPE DROPDOWN ================= */
            const unitSelect = document.getElementById("unitTypeSelect");
            if (unitSelect) {
                unitSelect.innerHTML = "";
                tabs.forEach(tab => {
                    const opt = document.createElement("option");
                    opt.value = tab.dataset.title || tab.textContent.trim();
                    opt.textContent = tab.dataset.title || tab.textContent.trim();
                    unitSelect.appendChild(opt);
                });
            }

            /* ================= SPEC by TYPE + CATEGORY ================= */
            const specMainImage = document.querySelector(".unit-spec-image img");

            // atas = tipe
            const typeBtns = Array.from(document.querySelectorAll(".spec-unit-btn"));
            // bawah = kategori
            const allCatBtns = Array.from(document.querySelectorAll(".spec-btn"));
            ...
            let activeType = (typeBtns[0]?.dataset?.type) || "";
            let currentBtns = [];
            let specIndex = 0;

            const titleOf = (btn) => (btn?.dataset?.title || btn?.textContent || "").trim();

            function showSpec(i) {
                if (!currentBtns.length) {
                    if (specMainImage) specMainImage.src = "";
                    if (prevLabel) prevLabel.textContent = "-";
                    if (nextLabel) nextLabel.textContent = "-";
                    return;
                }

                specIndex = (i + currentBtns.length) % currentBtns.length;

                const cur = currentBtns[specIndex];
                if (specMainImage && cur.dataset.img) specMainImage.src = cur.dataset.img;

                const prev = currentBtns[(specIndex - 1 + currentBtns.length) % currentBtns.length];
                const next = currentBtns[(specIndex + 1) % currentBtns.length];

                if (prevLabel) prevLabel.textContent = titleOf(prev) || "-";
                if (nextLabel) nextLabel.textContent = titleOf(next) || "-";
            }
            ...
            function applyType(type) {
                ...
                currentBtns = allCatBtns.filter(b => b.dataset.type === activeType);

                allCatBtns.forEach(b => {
                    b.style.display = (b.dataset.type === activeType) ? "" : "none";
                });
            ...
            allCatBtns.forEach(btn => {
                btn.addEventListener("click", (e) => {
                    ...
                    const idx = currentBtns.indexOf(btn);
                    if (idx >= 0) showSpec(idx);
                });
                ...
                btn.addEventListener("mouseenter", () => {
                    ...
                    previewTitle.textContent = titleOf(btn) || "-";
            ...
            // prev/next
            if (specNextBtn) specNextBtn.addEventListener("click", (e) => {
                e.preventDefault();
                showSpec(specIndex + 1);
            });

            if (specPrevBtn) specPrevBtn.addEventListener("click", (e) => {
                e.preventDefault();
                showSpec(specIndex - 1);
            });

            // init
            if (typeBtns.length) applyType(activeType);

            /* ================= NAVBAR: MOBILE MENU (contact.php style) ================= */
            ...
            if (navToggle && navMobile) {
                ...
                document.addEventListener("click", () => navMobile.classList.remove("show"));
            ...
            /* ================= NAVBAR: DESKTOP PROPERTY DROPDOWN ================= */
            ...
            /* ================= NAVBAR: MOBILE PROPERTY DROPDOWN ================= */
            ...
            /* ================= PROFILE MENU (contact.php style) ================= */
